...
1.Start
2.Insert or delete a node using BST rules
3.Update the height of the current node
4.Calculate Balance Factor (BF) = Height(Left) − Height(Right)
5.Check for imbalance:
● If BF > 1 → Left heavy
● If BF < -1 → Right heavy
6.Perform rotations based on case:
● LL Case (BF > 1 and key < left child) → Perform Right Rotation
● RR Case (BF < -1 and key > right child) → Perform Left Rotation
● LR Case (BF > 1 and key > left child) → Perform Left Rotation on left child, then Right Rotation
● RL Case (BF < -1 and key < right child) → Perform Right Rotation on right child, then Left Rotation
7.Repeat the process for all affected nodes up to the root
8.Stop
*/
